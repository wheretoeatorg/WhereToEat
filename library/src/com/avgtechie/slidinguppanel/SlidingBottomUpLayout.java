package com.avgtechie.slidinguppanel;
...
public class SlidingBottomUpLayout extends ViewGroup {

    private static final String TAG = "SlidingBottomUpLayout";
    ...
    public void setListViewFirstRowVisible(boolean isListViewFirstRowVisible) {
        Log.d(TAG, "isListViewFirstRowVisible :" + isListViewFirstRowVisible);
        this.isListViewFirstRowVisible = isListViewFirstRowVisible;
    }

    public boolean isExpandedPanel() {
        return isExpandedPanel;
    }

    public void setExpandedPanel(boolean isExpandedPanel) {
        this.isExpandedPanel = isExpandedPanel;
    }
    ...
    public SlidingBottomUpLayout(Context context) {
        this(context, null);
    }

    public SlidingBottomUpLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SlidingBottomUpLayout(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        if (attrs != null) {
            TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.SlidingBottomUpLayout);

            if (ta != null) {
                mPanelHeight = ta.getDimensionPixelSize(R.styleable.SlidingBottomUpLayout_collapsedHeight, -1);
                Log.d(TAG, "mPanelHeight : " + mPanelHeight);
                mShadowHeight = ta.getDimensionPixelSize(R.styleable.SlidingBottomUpLayout_shadowHeight, -1);

                mMinFlingVelocity = ta.getInt(R.styleable.SlidingBottomUpLayout_flingVelocity, DEFAULT_MIN_FLING_VELOCITY);
                mCoveredFadeColor = ta.getColor(R.styleable.SlidingBottomUpLayout_fadeColor, DEFAULT_FADE_COLOR);
                mDragViewResId = ta.getResourceId(R.styleable.SlidingBottomUpLayout_dragView, -1);
            }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        Log.d(TAG, "slidingTop : " + paddingTop);
        Log.d(TAG, "slidingTop : " + slidingTop);
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            final int childTop = lp.slideable ? slidingTop + (int) (mSlideRange * mSlideOffset)
                    : paddingTop;
...
//                }
                ...
                if (mIsUsingDragViewTouchEvents) {
                    Log.d(TAG, "onInterceptTouchEvent() ACTION_MOVE 703 ");
                    if (adx > mScrollTouchSlop && ady < mScrollTouchSlop) {
                        Log.d(TAG, "onInterceptTouchEvent() ACTION_MOVE 705 ");
                        return super.onInterceptTouchEvent(ev);
                    }
                ...
                if (ady > dragSlop && adx > ady) {
                    Log.d(TAG, "onInterceptTouchEvent() ACTION_MOVE 718 ");
                    mDragHelper.cancel();
    ...
    private class DragHelperCallback extends ViewDragHelper.Callback {
        /**
         * implement this method to allow which child view can be slidable.
         */
        @Override
        ...
        public void onViewDragStateChanged(int state) {
            ...
            Log.d(TAG, "onViewDragStateChanged mSlideOffset:" + mSlideOffset);
            Log.d(TAG,
                    "onViewDragStateChanged mDragHelper.getViewDragState():"
                            + mDragHelper.getViewDragState()
            );
            switch (state) {
            ...
            if (mDragHelper.getViewDragState() == ViewDragHelper.STATE_IDLE) {
                if (mSlideOffset == 0) {
                    Log.d(TAG, "onViewDragStateChanged mSlideOffset = 0");
                    if (mSlideState != SlideState.EXPANDED) {
                        Log.d(TAG, "onViewDragStateChanged mSlideState not Expanded");
                        updateObscuredViewVisibility();
                    ...
                    if (mSlideState != SlideState.ANCHORED) {
                        Log.d(TAG, "onViewDragStateChanged mSlideState not Anchored ");
                        updateObscuredViewVisibility();
                        dispatchOnPanelAnchored(mSlideableView);
                        mSlideState = SlideState.ANCHORED;
                    }
                } else if (mSlideState != SlideState.COLLAPSED) {
                    Log.d(TAG, "onViewDragStateChanged mSlideState not Collapsed");
                    dispatchOnPanelCollapsed(mSlideableView);
                    mSlideState = SlideState.COLLAPSED;
                }
            } else {
                Log.d(TAG, "onViewDragStateChanged mSlideState not IDLE");
            }
        }
